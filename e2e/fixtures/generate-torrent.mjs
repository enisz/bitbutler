...
// A separate torrent used by the add-torrent test so it is never pre-seeded by globalSetup
const addTorrent = {
  info: {
    name: 'test-add-files',
    'piece length': pieceLength,
    pieces,
    files: [{ length: Buffer.from('add').length, path: ['add.txt'] }],
  },
};

const addOutPath = join(__dirname, 'test-add.torrent');
writeFileSync(addOutPath, bencode(addTorrent));
console.log(`Written ${addOutPath} (${bencode(addTorrent).length} bytes)`);
