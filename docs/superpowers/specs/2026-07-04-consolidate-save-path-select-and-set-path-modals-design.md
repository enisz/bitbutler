...
- `handleSubmit`:
  ...
  - `pathType() === 'download'`: take the form value as-is (including `''`/`null`, no fallback chain, no blocking on empty). Call `qbService.torrents.setDownloadPath(serverId, hashes(), newPath ?? '')`. This is the fix for the "can't clear the download path" bug.
