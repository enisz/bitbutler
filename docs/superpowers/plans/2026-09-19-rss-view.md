...
- **Test environment:** the developer runs a throwaway qBittorrent 5.2.3 plus a local test RSS feed in Docker on the target machine (`docs/superpowers/test-env/`, start/stop one-liners in its README; login `admin` / `bitbutler-test`, WebUI on `http://127.0.0.1:8080`, feed `http://127.0.0.1:8081/torrents.xml`). It is already seeded with two feeds: a local "Test Feed" covering every code path (enclosure+category+size, magnet, news-style web link, `.torrent` link, textual size, HTML description) and "FOSS Torrents". RSS processing starts OFF so the disabled banner can be tested. The target machine may be a headless Ubuntu server (no GUI).
...
- [ ] **Step 1: Start the test qBittorrent and re-check the response shape**

The response shape below was already verified against a real qBittorrent 5.2.3 on 2026-09-19; a captured copy is in `docs/superpowers/test-env/sample-rss-items.json`. The developer starts a throwaway test qBittorrent (plus a local test feed) with one command, documented in `docs/superpowers/test-env/README.md`. Check it is up, then re-check the shape (default port 8080, login `admin` / `bitbutler-test`; a successful login is HTTP 204 with an empty body):

```bash
COOKIE="${TMPDIR:-/tmp}/qb.cookie"
curl -s -c "$COOKIE" -o /dev/null -w 'login %{http_code}
' -d 'username=admin&password=bitbutler-test' http://127.0.0.1:8080/api/v2/auth/login
curl -s -b "$COOKIE" -o /dev/null --data-urlencode 'json={"rss_processing_enabled":true}' http://127.0.0.1:8080/api/v2/app/setPreferences
curl -s -b "$COOKIE" "http://127.0.0.1:8080/api/v2/rss/items?withData=true" | head -c 3000
```

Expected: `login 204`, then a JSON tree whose feed nodes have `uid`, `url`, `title`, `hasError`, `isLoading`, `lastBuildDate`, `articles[]`; articles have `id`, `title`, `date` (like `19 Sep 2026 09:00:00 +0000`), `description`, and `link`/`torrentURL`, with `category`, `contentLength`, `size`, `author` only where the feed provides them, and no `isRead` until read. (Feeds may need a few seconds after enabling processing; `curl -s -b "$COOKIE" -d 'itemPath=Test Feed' http://127.0.0.1:8080/api/v2/rss/refreshItem` forces a refresh.) If the test environment is not available, continue with the shape below and say so in your final report.
...
**The target machine may be a headless Ubuntu server, where Electron (`npm start`) cannot run.** In that case do not attempt the GUI checks. Instead: (a) run all automated checks from Step 1; (b) verify the qB-facing behavior with `curl` against the test qBittorrent as in Task 1 Step 1 (feeds listed, `markAsRead`, `addFeed`/`moveItem`/`removeItem` round trips, `rss_processing_enabled` toggling); (c) in your final report list which numbered items below were verified and which were NOT, so the developer can run the GUI checks from a desktop machine, pointing BitButler at the test qBittorrent (`docs/superpowers/test-env/README.md` explains the SSH tunnel).
